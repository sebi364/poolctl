...
def pool_detail(pool):
    ...
    table.add_column("Memory (aloc.):", footer=f"{round(sum([i[4] for i in output]) / 2**30, 2)} GB")
    table.add_column("Memory (used):", footer=f"{round(sum([i[5] for i in output]) / 2**30, 2)} GB")
    table.add_column("Core (aloc.):", footer=f"{round(sum([i[6] for i in output]), 2)} CPU")
    table.add_column("Core (used):", footer=f"{round(sum([i[7] for i in output]), 2)} CPU")
    table.add_column("Storage:", footer=f"{round(sum([i[8] for i in output]) / 2**30, 2)} GB")
