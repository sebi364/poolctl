...
def configure():
    ...
    config["api_user"] = questionary.text(
        "What API user should poolctl use? [Example: root@pam!poolctl]",
        validate=match_user,
    ).ask()
    config["api_url"] = questionary.text(
        "What API endpoint should poolctl use? [Example: https://pvehost.lan:8006/api2/json/]",
        validate=match_api,
    ).ask()
    config["api_secret"] = questionary.password(
        "What API secret should poolctl use?"
    ).ask()
    config["api_verify"] = questionary.confirm(
    ...
    if r.status_code != 200:
        log.error(
            f"Connection was successfull, but the authentication failed: {r.status_code}"
        )
        log.error(r.text)
        exit(1)
    else:
